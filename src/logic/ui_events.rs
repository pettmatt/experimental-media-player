...
pub fn handle_events(app: &AppWindow, state: &mut Rc<RefCell<State>>) {
	...
	global_media_actions.on_media_start({
		...
		move |id: i32| {
			let temp_state = state_clone.borrow().index.clone();
			if let Some((index, media)) = temp_state.iter().enumerate()
				.find(|(_, item)| item.id == id) {
					audio_control_events::handle_media_start(&mut player_clone, media);
					state_clone.borrow_mut().add_to_queue(media);
	...
	global_media_actions.on_media_change_volume({
		let mut player_clone = Rc::clone(&player);
		move |volume: i32|
			audio_control_events::handle_media_volume(
				&mut player_clone, volume
			)
	});
	...
	global_media_actions.on_media_get_track_position({
		...
		move || {
			...
			state_clone.borrow_mut().playing.update_position(value as i32);
		}
	});
	...
	global_media_actions.on_add_to_playlist({
		...
		if let Some(app) = app_clone.upgrade() {
			state_clone.borrow_mut().add_to_playlist(playlist_id, media_id, &app.global::<SlintState>());
		}
	}});
}
